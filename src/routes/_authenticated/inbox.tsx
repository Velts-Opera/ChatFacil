...
function InboxPage() {
  ...
  const { data: conversations = [], isLoading: loadingConversations } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("conversations")
        .select(
          "id,status,ai_handling,channel,channel_id,last_message,unread_count,handoff_reason,last_message_at,contacts(id,name,phone,wa_id,notes)",
        )
        .order("last_message_at", { ascending: false })
        .limit(80);

      if (filter === "abertas") query = query.eq("status", "aberta");
      else if (filter === "pendentes") query = query.eq("status", "pendente");
      else if (filter === "resolvidas") query = query.eq("status", "resolvida");
      else if (filter === "ia") query = query.eq("ai_handling", true).neq("status", "resolvida");
      else if (filter === "humano") query = query.eq("ai_handling", false).neq("status", "resolvida");

      const { data, error } = await query;
      if (error) throw error;
      const list = ((data ?? []) as any[]).map((conversation) => ({
        ...conversation,
        contacts: Array.isArray(conversation.contacts) ? conversation.contacts[0] : conversation.contacts,
      })) as Conversation[];
  ...
  async function markAsHuman() {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        status: "pendente",
        unread_count: 0,
        handoff_reason: "Atendimento assumido manualmente",
      })
      .eq("id", selected.id);
  ...
  async function resumeAi() {
    ...
    const { error } = await supabase
      .from("conversations")
      .update({ ai_handling: true, status: "aberta", handoff_reason: null })
      .eq("id", selected.id);
    setResuming(false);
    if (error) return toast.error(error.message);
    toast.success("IA retomou o atendimento.");
    qc.invalidateQueries({ queryKey: ["conversations"] });
  }
  ...
  async function closeConversation() {
    ...
    const { error } = await supabase
      .from("conversations")
      .update({ status: "resolvida", ai_handling: false, unread_count: 0 })
      .eq("id", selected.id);
    if (error) return toast.error(error.message);
  ...
  async function sendReply() {
    if (!selected?.channel_id) return toast.error("Conversa sem canal conectado.");
  ...
  return (
    <div className="grid h-[calc(100vh-3.25rem)] grid-cols-1 md:h-screen md:grid-cols-[320px_1fr_280px]">
      <div className={cn("flex min-h-0 flex-col border-r border-border bg-card", mobileView === "chat" && "hidden md:flex")}>
        <div className="border-b border-border p-4">
          <div className="flex items-center justify-between">
            ...
            <Button variant="ghost" size="icon" onClick={() => qc.invalidateQueries({ queryKey: ["conversations"] })}>
              <RefreshCw className="h-4 w-4" />
            </Button>
          </div>
          <div className="relative mt-3">
            <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <Input placeholder="Buscar contato…" value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9" />
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {conversations.map((conversation) => (
            <button
              ...
              className={cn(
                "flex w-full items-start gap-3 border-b border-border/60 px-4 py-3 text-left hover:bg-muted/50",
                selectedId === conversation.id && "bg-muted/60",
              )}
            >
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary text-xs font-semibold text-primary-foreground">
                {(conversation.contacts?.name ?? "?").slice(0, 2).toUpperCase()}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  <span className="truncate text-sm font-medium">{conversation.contacts?.name ?? "Sem nome"}</span>
                  <span className="text-[10px] text-muted-foreground">WhatsApp</span>
                </div>
                <div className="mt-0.5 truncate text-xs text-muted-foreground">{conversation.last_message ?? "Sem mensagem"}</div>
                <div className="mt-1 flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  <span className="rounded-full bg-muted px-1.5 py-0.5">{conversation.status}</span>
                  {conversation.ai_handling && <span className="rounded-full bg-primary/10 px-1.5 py-0.5 text-primary">IA</span>}
                  {Number(conversation.unread_count ?? 0) > 0 && <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-red-700">{conversation.unread_count}</span>}
                </div>
              </div>
            </button>
          ))}
          ...
          <>
            <div className="flex items-center justify-between gap-3 border-b border-border bg-card px-4 py-3">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="grid h-10 w-10 place-items-center rounded-full bg-primary text-xs font-semibold text-primary-foreground">
                  {(selected.contacts?.name ?? "?").slice(0, 2).toUpperCase()}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {selected.ai_handling ? (
                  <Button size="sm" variant="outline" onClick={markAsHuman}><UserCheck className="mr-1 h-3.5 w-3.5" /> Assumir</Button>
                ) : (
                  <Button size="sm" variant="outline" onClick={resumeAi} disabled={resuming}>
                    {resuming ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <Bot className="mr-1 h-3.5 w-3.5" />} Retomar IA
                  </Button>
                )}
              ...
              {messages.map((message) => {
                ...
                return (
                  <div key={message.id} className={cn("flex", outbound ? "justify-end" : "justify-start")}>
                    <div className={cn("max-w-[78%] rounded-2xl px-3 py-2 text-sm shadow-sm", outbound ? "bg-primary text-primary-foreground" : "border border-border bg-card")}>
            ...
            <div className="border-t border-border bg-card p-3">
              <div className="flex items-end gap-2">
                ...
                />
                <Button size="icon" title="Enviar" onClick={sendReply} disabled={sending || !reply.trim()}>
                  {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}
                </Button>
              </div>
              <div className="mt-1 text-[10px] text-muted-foreground">Ctrl + Enter envia pela WhatsApp Cloud API.</div>
            </div>
          </>
        )}
      </div>
      ...
      <aside className="hidden min-h-0 flex-col border-l border-border bg-card md:flex">
        {selected ? (
          <div className="space-y-4 p-5">
            ...
            <InfoBlock label="Canal" value="WhatsApp" />
            <InfoBlock label="Última mensagem" value={selected.last_message ?? "—"} />
            {selected.contacts?.notes && <InfoBlock label="Observações" value={selected.contacts.notes} />}
          </div>
        ) : (
