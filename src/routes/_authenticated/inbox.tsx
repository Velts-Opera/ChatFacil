...
import { greetingForSaoPaulo } from "@/lib/greeting";
...
function InboxPage() {
  ...
  const { data: conversations = [], isLoading: loadingConversations } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("conversations")
        .select(
          "id, status, ai_handling, channel, channel_id, last_message, last_message_direction, unread_count, handoff_reason, last_message_at, contacts(id, name, phone, wa_id, funnel_stage, notes)",
        )
      ...
      return term
        ? list.filter((c) =>
            `${c.contacts?.name ?? ""} ${c.contacts?.phone ?? ""}`.toLowerCase().includes(term),
          )
        : list;
    },
  });
  ...
  const { data: messages = [], isLoading: loadingMessages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("messages")
        .select(
          "id, direction, sender_type, content, status, message_type, ai_generated, created_at",
        )
  ...
  const [markingEfetivado, setMarkingEfetivado] = useState(false);
  ...
  // Marca o lead como Efetivado no banco imediatamente e, em seguida, dispara um
  // agradecimento automático pelo WhatsApp — as duas regras de negócio da Etapa 5.
  async function markEfetivado() {
    if (!selected?.contacts?.id) return;
    setMarkingEfetivado(true);
    try {
      const { error } = await supabase
        .from("contacts")
        .update({ funnel_stage: "cliente_efetivado" })
        .eq("id", selected.contacts.id);
      if (error) throw error;
      toast.success("Lead marcado como Efetivado.");
      qc.invalidateQueries({ queryKey: ["conversations"] });

      if (selected.channel_id) {
        const name = selected.contacts?.name?.trim();
        const message = `${greetingForSaoPaulo()}${name ? `, ${name}` : ""}! Muito obrigado por fechar com a gente. Qualquer coisa, é só chamar por aqui. 🙌`;
        try {
          await sendWhatsAppMessage(selected.channel_id, { conversation_id: selected.id, message });
          qc.invalidateQueries({ queryKey: ["messages", selected.id] });
        } catch (sendError) {
          toast.error(
            `Lead efetivado, mas o agradecimento não foi enviado: ${formatWhatsAppApiError(sendError)}`,
          );
        }
      }
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Falha ao marcar como Efetivado.");
    } finally {
      setMarkingEfetivado(false);
    }
  }
  ...
  async function sendReply() {
    if (!selected?.channel_id) {
      toast.error("Conversa sem canal conectado.");
      return;
    }
            ...
            <div className="flex items-center justify-between gap-3 border-b border-border bg-card px-4 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {selected.ai_handling ? (
                  <Button size="sm" variant="outline" onClick={markAsHuman}>
                    <UserCheck className="mr-1 h-3.5 w-3.5" /> Assumir
                  </Button>
                ) : (
                ...
                <Button
                  size="sm"
                  variant="outline"
                  onClick={markEfetivado}
                  disabled={
                    markingEfetivado ||
                    !selected.contacts?.id ||
                    selected.contacts?.funnel_stage === "cliente_efetivado"
                  }
                >
                  {markingEfetivado ? (
                    <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" />
            ...
            <div className="border-t border-border bg-card p-3">
              <div className="flex items-end gap-2">
                ...
                <div className="flex flex-col gap-2">
                  <Button size="icon" variant="outline" title="Resposta rápida">
              ...
              <div className="mt-1 text-[10px] text-muted-foreground">
                Ctrl + Enter envia. A mensagem sai pela WhatsApp Cloud API real.
              </div>
            </div>
          </>
        )}
        ...
        {selected ? (
          <div className="space-y-4 p-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              <Badge variant={selected.ai_handling ? "default" : "outline"}>
                {selected.ai_handling ? "IA atendendo" : "Humano/pendente"}
              </Badge>
          ...
          <div className="grid flex-1 place-items-center p-6 text-center text-xs text-muted-foreground">
            Detalhes do contato aparecerão aqui.
          </div>
        )}
      </aside>
    </div>
