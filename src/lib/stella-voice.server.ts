...
const DEFAULT_LIVEKIT_URL = "wss://veltsapp-j8mqf7tp.livekit.cloud";
...
function getLiveKitUrl(): string | null {
  const value = (process.env.LIVEKIT_URL || DEFAULT_LIVEKIT_URL).trim();
  if (!/^wss:\/\/[A-Za-z0-9.-]+(?::[0-9]+)?(?:\/.*)?$/.test(value)) return null;
  return value;
}
