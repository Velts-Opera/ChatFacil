...
function aiConfig() {
  ...
  const model = (Deno.env.get("AI_MODEL") ?? Deno.env.get("OPENAI_MODEL") ?? (provider === "alibaba" ? "qwen-plus" : "gpt-4o-mini")).trim();
  ...
  return { apiKey, provider, model, url: base.toString() };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [{ data: settings, error: settingsError }, { data: company, error: companyError }, { data: template, error: templateError }] = await Promise.all([
      admin.from("ai_agent_settings").select("id, is_system_locked, prompt_version").eq("company_id", companyId).maybeSingle(),
      admin.from("companies").select("name, segment, business_hours, services_description, communication_tone").eq("id", companyId).maybeSingle(),
      admin.from("agent_profession_templates").select("slug, name, base_instructions, discovery_questions").eq("slug", profession).eq("is_active", true).maybeSingle(),
    ]);
    if (settingsError) throw settingsError;
    if (companyError) throw companyError;
    if (templateError) throw templateError;
    if (settings?.is_system_locked) return json({ error: "Este agente usa instruções fixas da operação e não pode ser sobrescrito pelo gerador." }, 409);

    let selectedTemplate = template;
    if (!selectedTemplate) {
      const { data, error } = await admin.from("agent_profession_templates").select("slug, name, base_instructions, discovery_questions").eq("slug", "geral").maybeSingle();
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `Dados para montar o agente:\n${JSON.stringify(source, null, 2)}` },
        ],
      }),
      signal: AbortSignal.timeout(20_000),
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(result?.error?.message ?? `Falha ao gerar o agente (HTTP ${response.status}).`);

    const generated = clean(result?.choices?.[0]?.message?.content, 24000);
    if (generated.length < 120) throw new Error("A IA não retornou um prompt válido. Tente novamente.");
    ...
    const { error: saveError } = await admin.from("ai_agent_settings").upsert(payload, { onConflict: "company_id" });
