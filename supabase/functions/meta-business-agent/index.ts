...
type Action =
  "status" | "check_eligibility" | "onboard" | "configure_from_chatfacil" | "set_rollout";
type RequestBody = {
  action?: Action;
  channel_id?: string;
  enabled?: boolean;
...
type MetaApiResult = { ok: boolean; status: number; body: any };
...
async function loadChannel(admin: any, companyId: string, channelId?: string) {
  ...
  if (!data.phone_number_id || !data.waba_id)
    throw new Error("Canal oficial sem Phone Number ID ou WABA ID");
  return data;
}
...
async function checkEligibility(admin: any, channel: any, accessToken: string) {
  ...
  if (!result.ok) {
    ...
    const termsRequired =
      result.status === 403 && /terms|termos|service must be accepted/i.test(message);
...
async function configureFromChatFacil(admin: any, channel: any, accessToken: string) {
  const [{ data: company, error: companyError }, { data: agent, error: agentError }] =
    await Promise.all([
      admin
        .from("companies")
  ...
  const description = [
    ...
    .filter(Boolean)
    .join("\n");
  const businessInfo = await metaAgentRequest(
    channel.phone_number_id,
    "agent_config/business_info",
    accessToken,
    {
      method: "PUT",
      body: JSON.stringify({
  ...
  if (instructions && channel.meta_business_agent_status === "onboarding") {
    const skill = await metaAgentRequest(
      channel.phone_number_id,
      "agent_config/skills",
      accessToken,
      {
  ...
  const settings = await metaAgentRequest(
    channel.phone_number_id,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!secret?.access_token)
      return json({ error: "Token oficial da Meta não encontrado para este canal" }, 409);
    await resolveBusinessPortfolioId(context.admin, channel, secret.access_token);
    if (body.action === "status" || !body.action)
      return json({
        ok: true,
        channel,
        manager_url: managerUrl(channel),
        api_version: businessAgentVersion(),
      });
    if (body.action === "check_eligibility") {
      const result = await checkEligibility(context.admin, channel, secret.access_token);
      return json(
        { ...result, manager_url: managerUrl(channel) },
        result.ok ? 200 : result.terms_required ? 409 : 502,
      );
    }
    if (body.action === "onboard") {
      ...
      if (!eligibility.ok)
        return json(
          { ...eligibility, manager_url: managerUrl(channel) },
          eligibility.terms_required ? 409 : 502,
        );
      if (eligibility.eligible === false)
        return json({ error: "Este número não está elegível para o Meta Business Agent" }, 409);
      const result = await metaAgentRequest(
        channel.phone_number_id,
        "agent_onboarding?channel=whatsapp",
        secret.access_token,
        { method: "POST" },
      );
      if (!result.ok) {
    ...
    if (body.action === "set_rollout") {
      if (typeof body.enabled !== "boolean")
        return json({ error: "enabled deve ser boolean" }, 400);
    ...
    return json(
      { error: message },
      /Unauthorized|Missing Authorization/i.test(message) ? 401 : 500,
    );
  }
});
