...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!secret?.access_token || !channel.phone_number_id)
      return json({ ok: false, error: "Credenciais ausentes." }, 200);
    const result = await metaGet(
      `/${encodeURIComponent(channel.phone_number_id)}?fields=id,display_phone_number,verified_name,quality_rating`,
      secret.access_token,
    );
    ...
    const errorMessage = result.ok
      ? null
      : (result.json?.error?.message ?? `Meta HTTP ${result.status}`);
    await admin
      .from("integration_health_checks")
      .insert({
        company_id: companyId,
        channel_id: channel.id,
        check_type: "meta_phone_number",
        status,
