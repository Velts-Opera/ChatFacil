...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!body.channel_id || !to || !body.template_name)
      return json({ error: "channel_id, to e template_name são obrigatórios." }, 400);
    ...
    if (channel.provider !== "meta_cloud_api")
      return json(
        { error: "Templates são enviados somente por canais oficiais da Meta Cloud API." },
        409,
      );
    ...
    const minutes = Math.max(
      ...
      now = new Date(),
      pauseUntil = new Date(now.getTime() + minutes * 60000).toISOString();
    await admin
      .from("conversations")
      .update({
        ai_handling: false,
