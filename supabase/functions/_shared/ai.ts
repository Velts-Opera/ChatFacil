...
function buildBiaSystemPrompt(company: any, channel: any, knowledge: any[], quickReplies: any[]) {
  const assistantName = company?.ai_assistant_name?.trim() || "Bia";
...
export async function generateBiaReply(
  ...
  const system = buildBiaSystemPrompt(company, channel, knowledge ?? [], quickReplies ?? []);
