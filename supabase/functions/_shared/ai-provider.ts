...
export function isAiAutoReplyEnabled(channel: {
...
export function resolveAiProviderConfig(env: AiProviderEnvironment): AiProviderConfig {
  ...
  if (
    ...
    throw new Error(
      "AI_BASE_URL deve ser uma URL HTTPS sem credenciais, porta customizada, query ou fragmento",
    );
  const normalizedPath = parsed.pathname.replace(/\/+$/, "");
  if (
    provider === "alibaba" &&
    (!isAlibabaModelStudioHost(parsed.hostname) || normalizedPath !== "/compatible-mode/v1")
  )
    throw new Error(
      "AI_BASE_URL da Alibaba deve usar um host oficial aliyuncs.com e terminar em /compatible-mode/v1",
    );
