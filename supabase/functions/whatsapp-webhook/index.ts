...
async function pauseForHuman(admin: any, conversationId: string, agentSettings: any, reason: string) {
...
async function generateAiReply(
  ...
  const [{ data: company }, { data: quickReplies }, { data: knowledge }, { data: history }] = await Promise.all([
    ...
    admin.from("ai_knowledge_items").select("title, content").eq("company_id", channel.company_id).eq("is_active", true).limit(50),
...
async function sendBotReply(
  ...
  if (!secret?.access_token || !channel.phone_number_id) {
    ...
    await admin.from("conversations").update({ ai_handling: false, status: "pendente", handoff_reason: "Canal sem credenciais Meta completas para envio" }).eq("id", conversationId);
    return;
  }
