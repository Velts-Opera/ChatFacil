...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!phoneNumberId || !wabaId || !verifyToken)
      return json(
        { error: "Nome, WABA ID, Phone Number ID e Verify Token são obrigatórios." },
        400,
      );
    ...
    if (!tokenToTest)
      return json({ error: "Access Token não encontrado. Informe um novo token." }, 400);
    ...
    const connectingPayload = {
      ...
      human_handoff_enabled:
        body.human_handoff_enabled ?? existingChannel?.human_handoff_enabled ?? true,
      handoff_when_unknown:
        body.handoff_when_unknown ?? existingChannel?.handoff_when_unknown ?? true,
      greeting_message:
        body.greeting_message ??
        existingChannel?.greeting_message ??
        "Olá! Recebemos sua mensagem. Vou te ajudar por aqui.",
      out_of_hours_message:
        body.out_of_hours_message ??
        existingChannel?.out_of_hours_message ??
        "Olá! Estamos fora do horário de atendimento. Já recebemos sua mensagem e responderemos assim que possível.",
      business_hours:
        body.business_hours ?? existingChannel?.business_hours ?? "Segunda a sexta, 09:00 às 18:00",
      created_by: user.id,
    };
    if (channelId) {
    ...
    if (!phoneResult.ok) {
      const errMsg =
        phoneResult.json?.error?.message || `Meta API retornou HTTP ${phoneResult.status}`;
      await admin
        .from("channels")
        .update({
          status: "error",
          last_error: errMsg,
          last_error_code: phoneResult.json?.error?.code
            ? String(phoneResult.json.error.code)
            : null,
