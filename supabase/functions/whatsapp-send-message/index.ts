...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (channel.provider === "qr_code") return json({ error: "Canais QR enviam exclusivamente pela API Railway autenticada." }, 409);
    ...
    const meta = await sendWhatsAppText(secret.access_token, channel.phone_number_id, to, body.message.trim(), AbortSignal.timeout(10_000));
    if (!meta.ok) {
    ...
    await admin.from("webhook_events").insert({
      ...
      status: "ok",
      source: "app",
      payload: { to, meta_message_id: metaMessageId, conversation_id: conversationId, ai_paused_until: pauseUntil },
