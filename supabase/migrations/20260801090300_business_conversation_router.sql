-- ChatFacil — deterministic WhatsApp scheduling conversation router.
-- The existing webhook remains authoritative. This AFTER INSERT trigger updates
-- conversation state before the webhook chooses an AI response, preventing a
-- duplicate generic answer while a scheduling flow is active.
...
AS $$
  SELECT trim(regexp_replace(
    lower(extensions.unaccent(COALESCE(_value,''))),
    '\s+',
    ' ',
    'g'
  ));
...
CREATE OR REPLACE FUNCTION public.business_parse_local_date(
  _value text,
...
DECLARE
  ...
  local_today date := (
    now() AT TIME ZONE COALESCE(NULLIF(_timezone,''),'America/Sao_Paulo')
  )::date;
  parts text[];
  day_value integer;
  month_value integer;
...
BEGIN
  ...
  IF parts IS NOT NULL THEN
    ...
    END;
    BEGIN
      IF make_date(year_value,month_value,day_value) < local_today
         AND parts[4] IS NULL THEN
...
CREATE OR REPLACE FUNCTION public.business_format_slot(
...
AS $$
  SELECT to_char(
    _starts_at AT TIME ZONE COALESCE(NULLIF(_timezone,''),'America/Sao_Paulo'),
...
BEGIN
  ...
  INSERT INTO public.outbound_queue(
    ...
    _conversation_id,
    _contact_id,
    regexp_replace(COALESCE(_to_phone,''),'\D','','g'),
    'text',
    jsonb_build_object(
      'message',trim(_message),
      'source','business_conversation'
    ),
    'queued',
    now(),
    _source_message_id
...
AS $$
  ...
  WHERE id=_job_id
    ...
    AND (
      business_opportunity_id IS NOT NULL
      OR payload->>'source'='business_conversation'
    )
  RETURNING *;
$$;
...
BEGIN
  ...
  ELSIF step='confirm_cancel' THEN
    IF normalized~'^(sim|s|confirmo|pode cancelar|cancelar)$' THEN
      ...
      FROM public.business_cancel_appointment(
        (state->>'appointment_id')::uuid,
        'Cancelado pelo cliente no WhatsApp'
      );
      state:='{}'::jsonb;
      reply:='Agendamento cancelado: '||upcoming.title||' em '||
        public.business_format_slot(upcoming.starts_at,settings_row.timezone)||'.';
  ...
  ELSIF step='choose_service' THEN
    IF normalized~'^\d{1,2}$' THEN
      ...
      FROM(
        SELECT bs.*,row_number() OVER(ORDER BY bs.name)::integer option_number
    ...
    ELSE
      ...
      reply:='Ótimo. Para qual dia você quer agendar '||selected_service.name||
        '? Pode responder, por exemplo: amanhã, sexta ou 15/08.';
    END IF;

  ELSIF step='choose_date' THEN
    SELECT * INTO selected_service
    FROM public.business_services
    WHERE id=(state->>'service_id')::uuid
    ...
    ELSE
      ...
      ELSE
        ...
        SELECT
          ...
          string_agg(
            option_number||'. '||to_char(
              slot_start AT TIME ZONE settings_row.timezone,
              'HH24:MI'
            )||CASE
              WHEN resource_name IS NULL THEN ''
              ELSE ' — '||resource_name
            END,
            E'\n' ORDER BY option_number
          )
        ...
        ELSE
          ...
          reply:='Horários disponíveis para '||selected_service.name||' em '||
            to_char(requested_date,'DD/MM')||':'||E'\n'||slots_text||E'\n'||
  ...
  ELSIF step='choose_slot' THEN
    ...
    ELSE
      ...
      WHERE (value->>'option')::integer=choice
      LIMIT 1;
      ...
      ELSE
        ...
        BEGIN
          SELECT * INTO booked
          FROM public.business_book_appointment(
            NEW.company_id,
            NEW.contact_id,
            selected_service.id,
          ...
          state:='{}'::jsonb;
          reply:='Agendamento confirmado: '||booked.title||' em '||
            public.business_format_slot(booked.starts_at,settings_row.timezone)||'.'||
            CASE
              WHEN booked.price_cents>0 THEN
                ' Valor: R$ '||to_char(booked.price_cents/100.0,'FM999G999G990D00')||'.'
              ELSE ''
            END;
        EXCEPTION WHEN others THEN
          state:=jsonb_build_object(
  ...
  ELSIF normalized~'(^|\W)(desmarcar|cancelar agendamento|cancelar horario)(\W|$)' THEN
    ...
    ELSE
      ...
      reply:='Encontrei '||upcoming.title||' em '||
        public.business_format_slot(upcoming.starts_at,settings_row.timezone)||
  ...
  ELSIF normalized~'(^|\W)(confirmar agendamento|confirmar horario)(\W|$)' THEN
    ...
    ELSE
      ...
      RETURNING * INTO upcoming;
      state:='{}'::jsonb;
      reply:='Agendamento confirmado: '||upcoming.title||' em '||
        public.business_format_slot(upcoming.starts_at,settings_row.timezone)||'.';
    END IF;
  ...
  ELSIF normalized~'(^|\W)(agendar|marcar|reservar|ver horario|ver horarios|horario disponivel|horarios disponiveis)(\W|$)' THEN
    ...
    ELSE
      ...
      reply:='Para qual dia você quer agendar '||selected_service.name||
        '? Pode responder: amanhã, sexta ou 15/08.';
    END IF;
  ELSE
    RETURN NEW;
  END IF;
  ...
  PERFORM public.business_queue_conversation_reply(
    ...
    NEW.contact_id,
    COALESCE(
      (SELECT NULLIF(wa_id,'') FROM public.contacts WHERE id=NEW.contact_id),
