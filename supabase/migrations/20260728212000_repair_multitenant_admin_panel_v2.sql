-- Repair objects from the original admin-panel migration that were missing in production.
...
CREATE POLICY "super admin - select companies" ON public.companies FOR SELECT TO authenticated USING (public.is_super_admin());
DROP POLICY IF EXISTS "super admin - update companies" ON public.companies;
CREATE POLICY "super admin - update companies" ON public.companies FOR UPDATE TO authenticated USING (public.is_super_admin()) WITH CHECK (public.is_super_admin());
DROP POLICY IF EXISTS "super admin - delete companies" ON public.companies;
CREATE POLICY "super admin - delete companies" ON public.companies FOR DELETE TO authenticated USING (public.is_super_admin());
...
CREATE POLICY "appointments - all own" ON public.appointments FOR ALL TO authenticated
  USING (company_id = (SELECT public.get_user_company_id()))
  WITH CHECK (company_id = (SELECT public.get_user_company_id()));
DROP TRIGGER IF EXISTS trg_appointments_updated_at ON public.appointments;
CREATE TRIGGER trg_appointments_updated_at BEFORE UPDATE ON public.appointments FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
AS $$
BEGIN
  IF NOT public.is_super_admin() THEN RAISE EXCEPTION 'Apenas o administrador da plataforma pode trocar de empresa'; END IF;
...
AS $$
  SELECT c.id, c.name, c.segment, c.plan, c.is_active, c.contact_name, c.phone, c.email, c.created_at,
    ch.status, ch.phone_number,
    COALESCE(ag.is_enabled, false),
    COALESCE(NULLIF(TRIM(ag.system_prompt), '') IS NOT NULL, false),
    (SELECT count(*) FROM public.ai_knowledge_items k WHERE k.company_id = c.id AND k.is_active),
    ...
    (SELECT count(*) FROM public.conversations cv WHERE cv.company_id = c.id)
