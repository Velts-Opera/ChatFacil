-- ChatFacil — business automation core
-- Real services, resources, hours, recurrence, opportunities and queue linkage.
...
CREATE TABLE IF NOT EXISTS public.business_services(
  ...
  recurrence_days integer CHECK(recurrence_days BETWEEN 1 AND 730),
  active boolean NOT NULL DEFAULT true,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);
CREATE UNIQUE INDEX IF NOT EXISTS uniq_business_services_company_name
  ON public.business_services(company_id,lower(name));
CREATE INDEX IF NOT EXISTS idx_business_services_company_active
  ON public.business_services(company_id,active);
...
CREATE POLICY "business services own" ON public.business_services
  FOR ALL TO authenticated
  USING(public.business_can_access_company(company_id))
  WITH CHECK(public.business_can_access_company(company_id));
...
CREATE UNIQUE INDEX IF NOT EXISTS uniq_business_resources_company_name
  ON public.business_resources(company_id,lower(name));
ALTER TABLE public.business_resources ENABLE ROW LEVEL SECURITY;
GRANT SELECT,INSERT,UPDATE,DELETE ON public.business_resources TO authenticated;
GRANT ALL ON public.business_resources TO service_role;
DROP POLICY IF EXISTS "business resources own" ON public.business_resources;
CREATE POLICY "business resources own" ON public.business_resources
  FOR ALL TO authenticated
  USING(public.business_can_access_company(company_id))
  WITH CHECK(public.business_can_access_company(company_id));
DROP TRIGGER IF EXISTS trg_business_resources_updated_at ON public.business_resources;
CREATE TRIGGER trg_business_resources_updated_at
...
GRANT SELECT,INSERT,UPDATE,DELETE ON public.business_resource_services TO authenticated;
GRANT ALL ON public.business_resource_services TO service_role;
DROP POLICY IF EXISTS "business resource services own" ON public.business_resource_services;
CREATE POLICY "business resource services own" ON public.business_resource_services
  FOR ALL TO authenticated
  USING(EXISTS(
    SELECT 1 FROM public.business_resources r
    WHERE r.id=resource_id AND public.business_can_access_company(r.company_id)
  ))
  WITH CHECK(EXISTS(
    SELECT 1 FROM public.business_resources r
    WHERE r.id=resource_id AND public.business_can_access_company(r.company_id)
...
CREATE TABLE IF NOT EXISTS public.business_hours(
  ...
  UNIQUE(company_id,weekday,opens_at,closes_at)
);
CREATE INDEX IF NOT EXISTS idx_business_hours_company_weekday
  ON public.business_hours(company_id,weekday,active);
ALTER TABLE public.business_hours ENABLE ROW LEVEL SECURITY;
GRANT SELECT,INSERT,UPDATE,DELETE ON public.business_hours TO authenticated;
GRANT ALL ON public.business_hours TO service_role;
DROP POLICY IF EXISTS "business hours own" ON public.business_hours;
CREATE POLICY "business hours own" ON public.business_hours
  FOR ALL TO authenticated
  USING(public.business_can_access_company(company_id))
  WITH CHECK(public.business_can_access_company(company_id));
...
CREATE TABLE IF NOT EXISTS public.business_automation_settings(
  ...
  empty_slot_enabled boolean NOT NULL DEFAULT false,
  reminders_enabled boolean NOT NULL DEFAULT true,
  reactivation_template_name text,
  empty_slot_template_name text,
...
GRANT ALL ON public.business_automation_settings TO service_role;
DROP POLICY IF EXISTS "business automation settings own" ON public.business_automation_settings;
CREATE POLICY "business automation settings own" ON public.business_automation_settings
  FOR ALL TO authenticated
  USING(public.business_can_access_company(company_id))
  WITH CHECK(public.business_can_access_company(company_id));
DROP TRIGGER IF EXISTS trg_business_automation_settings_updated_at ON public.business_automation_settings;
CREATE TRIGGER trg_business_automation_settings_updated_at
...
ALTER TABLE public.appointments
  ...
  ADD COLUMN IF NOT EXISTS resource_id uuid REFERENCES public.business_resources(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_appointments_company_resource_time
  ON public.appointments(company_id,resource_id,starts_at,ends_at);
CREATE INDEX IF NOT EXISTS idx_appointments_contact_status_time
  ON public.appointments(contact_id,status,starts_at DESC);

ALTER TABLE public.contacts
  ADD COLUMN IF NOT EXISTS automation_opt_out boolean NOT NULL DEFAULT false,
...
CREATE INDEX IF NOT EXISTS idx_contacts_company_expected_return
  ON public.contacts(company_id,expected_return_at)
...
CREATE TABLE IF NOT EXISTS public.business_opportunities(
  ...
  resource_id uuid REFERENCES public.business_resources(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_business_opportunities_company_status
  ON public.business_opportunities(company_id,status,detected_at DESC);
...
CREATE POLICY "business opportunities own" ON public.business_opportunities
  FOR ALL TO authenticated
  USING(public.business_can_access_company(company_id))
  WITH CHECK(public.business_can_access_company(company_id));
DROP TRIGGER IF EXISTS trg_business_opportunities_updated_at ON public.business_opportunities;
CREATE TRIGGER trg_business_opportunities_updated_at
  BEFORE UPDATE ON public.business_opportunities
  FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();

ALTER TABLE public.outbound_queue
  ADD COLUMN IF NOT EXISTS business_opportunity_id uuid REFERENCES public.business_opportunities(id) ON DELETE SET NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uniq_outbound_queue_business_opportunity
  ON public.outbound_queue(business_opportunity_id)
...
CREATE OR REPLACE FUNCTION public.seed_business_defaults(_company_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
AS $$
BEGIN
  INSERT INTO public.business_automation_settings(company_id)
  VALUES(_company_id)
  ON CONFLICT(company_id) DO NOTHING;

  INSERT INTO public.business_resources(company_id,name,kind)
  VALUES(_company_id,'Atendimento','professional')
  ON CONFLICT DO NOTHING;

  INSERT INTO public.business_hours(company_id,weekday,opens_at,closes_at)
  SELECT _company_id,d,time '09:00',time '18:00'
  FROM generate_series(1,5)d
  ON CONFLICT DO NOTHING;
END;
$$;
REVOKE EXECUTE ON FUNCTION public.seed_business_defaults(uuid) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.seed_business_defaults(uuid) TO service_role;

CREATE OR REPLACE FUNCTION public.tg_seed_business_defaults()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path=public
...
CREATE TRIGGER trg_companies_seed_business_defaults
  AFTER INSERT ON public.companies
  FOR EACH ROW EXECUTE FUNCTION public.tg_seed_business_defaults();
